...
if (valid.value.futureRootField?.version !== 2 || valid.value.accounts[0].futureAccountField !== true || valid.value.investments[0].futureInvestmentField !== true || valid.value.liabilities[0].futureLiabilityField !== true) {
...
if (summary.accountBalance !== 5000 || summary.investmentBalance !== 1000 || summary.cardBalance !== 1250 || summary.liabilityBalance !== 100 || summary.netPosition !== 4650) {
...
if (!/<dialog id="cardDialog"[^>]*data-static-backdrop/.test(html)) {
    throw new Error("Add Card dialog can still be dismissed by clicking its backdrop.");
}
if (!/<dialog id="liabilityDialog"[^>]*data-static-backdrop/.test(html)) {
